...
**2. Lógica em `admin_views.py` (`area_do_site_view`, action `criar_botao`)**:
   - Se **nenhuma** categoria existente foi escolhida **e** o checkbox foi marcado → cria uma `Categoria` de verdade, com slug único, aninhada em `Categoria(slug='botoes-novos-criados')` (criada via `get_or_create`, mesmo bucket oculto que `_criar_no()` do Painel Central e `_api_criar()` da Estrutura de Árvores já usam quando nenhum pai é escolhido) — `mostrar_menu_superior=False`, `mostrar_navegue_area=False` (não duplica na barra/home, já que o botão já aparece via a própria coluna extra)
...
- **Testado fim-a-ponta**: (a) botão SEM marcar checkbox → `categoria=None`, `link_externo` preenchido, comportamento antigo intacto ✓; (b) botão COM checkbox marcado + URL rápida + anexo rápido → nova `Categoria` criada dentro de "Botões novos criados", `mostrar_menu_superior=False`/`mostrar_navegue_area=False` ✓, `Conteudo` tipo link criado dentro dela ✓, `Anexo` criado dentro dela ✓, categoria **aparece na árvore JSON da Estrutura de Árvores** (`/admin/estrutura-arvores/api/?action=arvore_json`) ✓, botão na home aponta para `/categoria/<slug>/` corretamente ✓

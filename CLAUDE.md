# Site Currículos SEDU — Contexto do Projeto (v23 — atualizado em 2026-07-18 — Parte 27)
...
- **Migrações aplicadas**: `conteudo/0012-0032` + `painel/0002-0003` + `inteligencia/0002`. Migração **`conteudo/0032`** (textos de ajuda do Banner — ajuste automático) é a mais recente.
...
- **Altura fixa (opcional)**: preencher `altura_personalizada` liga a classe `.hero-slide--fixo`/`.cat-banner-item--fixo` → imagem inteira dentro da faixa (`object-fit:contain`) + fundo desfocado (`::before` blur) preenchendo as sobras.
...
- **Cache-busting**: `base.html` carrega `style.css` com `?v=AAAAMMDD-N` (hoje `20260718-1`) e `main.js` com `?v=20260711-1`. **Sempre incrementar ao mudar CSS/JS**, senão o navegador usa a versão cacheada. Ao testar: Ctrl+Shift+R.

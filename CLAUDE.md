...
- **Migrações aplicadas**: `conteudo/0012-0041` + `painel/0002-0003` + `inteligencia/0002`. Migração **`conteudo/0041`** (campos `icone_largura`/`icone_altura` em `ColunaExtra` — o ÍCONE DO TÍTULO da coluna, 2ª extensão da parte 37, 2026-07-22) é a mais recente. Anterior a ela: **`conteudo/0040`** (campos `icone_largura`/`icone_altura` em `ColunaExtraBotao` — os botões DENTRO da coluna, 1ª extensão da parte 37, 2026-07-22). Anterior a ela: **`conteudo/0039`** (campos `icone_largura`/`icone_altura` em Categoria, parte 37, 2026-07-22). Anterior a ela: **`conteudo/0038`** (todos os campos `URLField` sem `max_length` explícito passaram para 1000 chars — corrige `DataError` no sync com Postgres/Docker, links longos como SharePoint). Anterior a ela: **`conteudo/0037`** (`Anexo.url` novo campo + `arquivo`/`nome` viram opcionais, parte 36, 2026-07-22).
...
- **`icone_largura`/`icone_altura`** (PositiveIntegerField, px, opcionais, migração 0039 — parte 37, 2026-07-22) — controla o TAMANHO do ícone daquele botão específico (Font Awesome ou imagem), sobrescrevendo o tamanho padrão do CSS. Property **`icone_estilo_inline`** monta o `style="width:Npx;height:Npx;font-size:Npx;"` pronto (só largura preenchida = ícone quadrado; font-size calculado proporcionalmente para o glifo Font Awesome também escalar). Vazio (padrão) = tamanho normal do site, sem nenhuma mudança visual. Editável em **5 pontos administrativos**: Django Admin (fieldset "Aparência"), Painel Central (Criar novo botão, Criar subárea, Editar botão selecionado), Organizador (Criar novo botão dentro de X), Estrutura de Árvores (modal Criar botão + seção "Ícone" do painel de edição), Área do Site (botão completo do site). **`ColunaExtraBotao`** (modelo dos botões de "colunas extras", ver Painel "Área do Site") ganhou os MESMOS 2 campos + property (migração `conteudo/0040`) — editável no formulário "Adicionar botão" de cada coluna (campos sempre visíveis) e via botão "🔍" em cada botão já criado (mini-formulário inline, action `editar_botao_coluna`).

...
- **Leva mais recente (2026-07-22 — "parte 37")**: **2 pedidos do Dan resolvidos** — (1) **Sync de mídia Docker corrigido**: `ATUALIZAR BANCO DOCKER.bat` só sincronizava o BANCO (dumpdata/loaddata) — os ARQUIVOS de mídia (ícones, imagens, banners, anexos) nunca eram copiados, porque `docker-compose.yml` usa um volume Docker separado (`media_data:/app/media`) para a mídia dentro do container, isolado da pasta `media\` local. Resultado: ícone enviado localmente aparecia quebrado no ambiente Docker depois do sync, porque o banco (com a referência ao arquivo) ia, mas o arquivo em si, não. Corrigido: novo passo `[3/6]` no `.bat` que roda `docker compose cp media\. web:/app/media/` — copia (sem apagar nada já existente no Docker) todo o conteúdo da pasta `media\` local para dentro do container antes de aplicar as migrações/importar o banco. Não mexe no `docker-compose.yml` nem em nada mais — só adiciona esse passo de cópia. (2) **Tamanho do ícone dos botões configurável**: novos campos `Categoria.icone_largura`/`icone_altura` (px, opcionais, migração `conteudo/0039`) — quando preenchidos, controlam o tamanho do ícone daquele botão específico (Font Awesome ou imagem), sobrescrevendo o tamanho padrão do CSS via `style` inline (property `icone_estilo_inline`). Adicionado em **5 pontos administrativos**: Django Admin (fieldset "Aparência" de Categoria), Painel Central (Criar novo botão, Criar subárea, Editar botão selecionado), Organizador (Criar novo botão dentro de X), Estrutura de Árvores (modal Criar botão + seção "Ícone" do painel de edição), Área do Site (botão completo do site). Aplicado nos templates onde o ícone de um botão aparece: "Navegue por área" e área central da home, índice geral (`topic-icon`), subbotões (chips e cards), "Conteúdos recentes" (quando um botão aparece lá) e busca. **Fora do escopo**: os botões dentro de "colunas extras" (`ColunaExtraBotao`, painel Área do Site) usam um modelo diferente e não ganharam esse controle nesta leva — se o Dan precisar, é um pedido separado. Testado ponta a ponta via Django test client nos 5 pontos + confirmado no HTML renderizado da home que o `style="width:60px;height:60px;..."` aparece corretamente no ícone. Zero breaking changes — campos vazios (padrão) mantêm o tamanho de ícone exatamente como sempre foi.
...
- **Migrações aplicadas**: `conteudo/0012-0039` + `painel/0002-0003` + `inteligencia/0002`. Migração **`conteudo/0039`** (campos `icone_largura`/`icone_altura` em Categoria, parte 37, 2026-07-22) é a mais recente. Anterior a ela: **`conteudo/0038`** (todos os campos `URLField` sem `max_length` explícito passaram para 1000 chars — corrige `DataError` no sync com Postgres/Docker, links longos como SharePoint). Anterior a ela: **`conteudo/0037`** (`Anexo.url` novo campo + `arquivo`/`nome` viram opcionais, parte 36, 2026-07-22).

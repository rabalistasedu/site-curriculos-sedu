# Site Currículos SEDU — Contexto do Projeto (v50 — atualizado em 2026-07-24 — Parte 50: Exclusão em massa de itens na Lixeira)

> 📌 **Para conversas futuras**: leia [`memory/COMECE_AQUI.md`](memory/COMECE_AQUI.md) + [`memory/STATUS_FINAL_V42_2026_07_23.md`](memory/STATUS_FINAL_V42_2026_07_23.md) + [`memory/known_pitfalls.md`](memory/known_pitfalls.md) em ordem. Detalhes técnicos aqui; resumos e links rápidos em memory.
...
- **Leva mais recente (2026-07-24 — "parte 50")**: **Exclusão em massa de itens na Lixeira** — painel `/admin/lixeira/` agora tem recurso de **selecionar múltiplos botões/conteúdos e excluir de verdade todos de uma vez**. Cada tabela (Botões excluídos / Conteúdos excluídos) ganhou: (1) checkbox por linha + "Selecionar todos" (checkbox no cabeçalho) que marca/desmarca tudo automaticamente; (2) botão "Excluir selecionados definitivamente" que fica **desabilitado até marcar pelo menos 1 item** (feedback visual claro); (3) nova action no backend (`excluir_definitivo_massa`) que recebe lista de IDs e faz hard delete em lote de todos eles com uma única confirmação — sem precisar clicar "Excluir já" um por um. Os botões individuais "Restaurar" e "Excluir já" continuam funcionando exatamente como antes (zero mudança). Testado ponta a ponta no navegador: marcação de 2 itens de teste com "Selecionar todos" → botão ativado → confirmação → ambos excluídos com mensagem "2 botão(ões) excluído(s) definitivamente" ✓. **Zero breaking changes** — nenhuma mudança em outras telas, nenhuma migração de banco necessária (usa ação via POST no modelo existente `Categoria`/`Conteudo`, ambos com método `hard_delete()` já implementado na Parte 41). **Onde mexer**: `conteudo/admin_views.py` (nova action `excluir_definitivo_massa` na view `lixeira_view`) e `templates/admin/lixeira.html` (checkboxes + botão + JS helpers `lxToggleTodos`/`lxAtualizarBotaoMassa`).

- **Leva anterior (2026-07-24 — "parte 49")**: **Anexos de imagem em Categoria (botão) viram galeria visual, lado a lado, em vez de link de download**. O Dan anexou uma imagem (`jovem senador.jpg`) ao botão "ola mundo" via o novo painel Páginas Livres e reportou que ela aparecia como um link em "Arquivos para download" (ícone + "Clique para abrir ou baixar"), quando o esperado era a imagem aparecer visualmente na área de conteúdo do botão (a mesma área que mostra "Nenhum conteúdo nesta categoria ainda" quando vazia) — e, se houver várias, lado a lado. **Antes de implementar, checado o impacto**: como `Anexo` é um modelo compartilhado (usado por qualquer painel — Organizador, Estrutura de Árvores, Django Admin, Painel Central, e agora Páginas Livres), essa mudança afeta a renderização de `categoria.html`, usado por TODAS as 132 categorias do site. Verificado que só existiam 2 anexos de imagem ligados a categoria em todo o banco — e ambos eram os próprios botões de teste do Dan (`icone-teste` e `ola-mundo`) — confirmando que a mudança é segura e não afeta nenhum conteúdo real de produção. **Implementado**: nova property `Anexo.eh_imagem` (`conteudo/models.py`) detecta arquivos de imagem enviados (jpg/jpeg/png/gif/webp/bmp/svg — **anexos de link não contam**, só arquivos de verdade). A view `categoria_detalhe` (`conteudo/views.py`) agora separa os anexos da categoria em duas listas: `anexos_imagens` (renderizados na nova galeria) e `anexos_categoria` (o restante, continua exatamente igual em "Arquivos para download"). Novo bloco `.anexo-imagens-grid` em `templates/categoria.html`, posicionado logo antes da grade de conteúdo/subbotões — grid responsivo (`repeat(auto-fill, minmax(160px,1fr))`), cada imagem num quadrado (`aspect-ratio:1/1`, `object-fit:cover`), clicável para abrir a imagem em tamanho real numa aba nova. A mensagem "Nenhum conteúdo nesta categoria ainda" só aparece agora quando não há subcategorias, conteúdos **e** imagens (`{% elif not anexos_imagens %}`). Novo CSS em `static/css/style.css` (`.anexo-imagens-grid`/`.anexo-imagem-item`), cache-busting incrementado para `?v=20260724-2`. **Testado ponta a ponta**: categoria `ola-mundo` (com a imagem real do Dan) → galeria aparece, imagem não aparece mais como link de download, mensagem de "vazio" não aparece mais ✓; categoria com anexo que NÃO é imagem (ex.: PDF) → continua 100% igual, sem galeria, "Arquivos para download" intacto ✓; regressão confirmada em home/categoria índice/busca/página livre, todas 200 ✓; confirmado visualmente no navegador (imagem renderizada como quadrado 178×178px, sem link, sem mensagem de vazio) ✓. **Escopo**: mudança feita só em `categoria.html` (páginas de botão) — `conteudo_detalhe.html` (páginas de conteúdo individual) não foi tocado, já que o pedido do Dan foi especificamente sobre anexar imagem num botão. **Zero breaking changes**: nenhuma categoria real do site tinha anexo de imagem antes desta mudança (só os 2 botões de teste do próprio Dan), então nenhum conteúdo de produção mudou de aparência — daqui pra frente, sempre que uma imagem for anexada a um botão (por qualquer painel), ela aparecerá na galeria em vez de como link.
- **Leva anterior (2026-07-24 — "parte 48", correção + extensão da Parte 47)**: **Páginas Livres — botão "morto" corrigido + arquivos/links direto no painel para cada botão**. Depois de usar o painel novo, o Dan reportou dois problemas: (1) criou um botão ("ola mundo") sem marcar "criar como botão completo do site" e sem preencher link externo — o botão ficou **sem nenhum destino** (`categoria=None`, `link_externo=''`), então não tinha como visualizá-lo em nenhum lugar nem anexar arquivos; (2) o painel Páginas Livres não tinha, para nenhum botão (nem os já vinculados a categorias existentes), uma forma de anexar arquivos/links sem sair do painel. **Correção 1 (bug de botão sem destino)**: a action `criar_botao` agora detecta quando nada foi escolhido (nem categoria existente, nem link externo, nem o checkbox marcado) e **automaticamente cria a categoria completa** nesse caso — um botão sempre tem que ter para onde ir, então não faz sentido permitir um botão "morto". O botão real do Dan (`ola mundo`) foi corrigido manualmente com o mesmo tratamento (criada a categoria `ola mundo`, pk 386, slug `ola-mundo`, e o botão religado a ela). **Correção 2 (arquivos/links por botão no painel)**: cada botão da lista que tenha uma `categoria` vinculada (seja uma já existente ou uma criada como "completo do site") agora tem, na própria tela de Páginas Livres: um badge com contador (📎 N) que abre um painel expansível listando os anexos/links já existentes daquela categoria (com botão de excluir cada um), uma zona de arrastar-e-soltar para subir vários arquivos de uma vez, e linhas dinâmicas de "mais links" — tudo reaproveitando os helpers já existentes (`_criar_links_extra`, `_criar_anexos_link`, `dropzone.js`, mesmo padrão usado em Painel Central/Organizador/Área do Site) e as novas actions `adicionar_conteudo_botao`/`excluir_conteudo_botao`. Também foi adicionado, ao lado do badge, um link direto **"Abrir no Organizador"** (`/admin/organizar/?cat=<pk>`) para o Dan acessar o gerenciamento completo daquela categoria (mover, criar subcategorias, editar conteúdos) sem precisar procurar o botão manualmente em outra árvore — confirmado que o Organizador carrega qualquer categoria por PK independente das flags de visibilidade (`mostrar_menu_superior`/`mostrar_navegue_area`), então botões "escondidos" (como os criados via Páginas Livres, que nascem com essas flags desligadas) sempre são encontráveis por esse link. **Testado ponta a ponta**: criar botão sem preencher nada agora gera categoria automaticamente ✓, painel mostra badge com contagem correta (anexos + links) ✓, upload de arquivo + link via o novo formulário aparecem imediatamente na página real da categoria (`/categoria/<slug>/`, seção "Arquivos para download") ✓, exclusão de anexo (permanente) e de link (via lixeira, `Conteudo.delete()` já soft-deleta desde a Parte 41) funcionam ✓, link "Abrir no Organizador" carrega a categoria certa mesmo sendo uma categoria "escondida" dos menus ✓, botão real do Dan (`ola mundo`) confirmado com categoria própria, painel funcionando, página `/categoria/ola-mundo/` carregando ✓. **Zero breaking changes**: a mudança na action `criar_botao` só afeta o caso que antes gerava um botão inútil (sem categoria nem link) — quem já preenche link externo ou escolhe categoria continua exatamente igual; as 2 novas actions (`adicionar_conteudo_botao`/`excluir_conteudo_botao`) são inteiramente novas, não alteram nenhuma outra tela do site (Anexo/Conteudo tipo=link continuam sendo os mesmos modelos já usados em toda parte, então o que for anexado aqui aparece exatamente igual em qualquer outro painel — Organizador, Estrutura de Árvores, Django Admin).
- **Leva anterior (2026-07-24 — "parte 47")**: **Novo painel "Páginas Livres" (`/admin/paginas-livres/`, banner roxo no dashboard)** — o Dan pediu uma forma de criar páginas em branco, no mesmo padrão visual do site, com botões (já existentes ou novos) e comentários no rodapé. **Antes de implementar, foi feita uma conversa de design** (2 perguntas via pergunta direta ao Dan): (1) o recurso de "adicionar imagem/arquivos dentro da página" — confirmado que é decorativo (não uma lista de anexos para download) — **na prática esse pedido acabou não sendo necessário implementar nesta leva**, já que o foco final ficou em texto + botões + comentários, sem imagem de fundo na página em si (diferente da barra `home-intro` da Parte 45, que já tem isso); (2) onde as páginas apareceriam no site — Dan escolheu a **Opção A: não aparecem sozinhas em nenhum menu automaticamente; o link é colocado manualmente onde o Dan quiser** (num botão de categoria com URL externa, numa coluna extra, no rodapé, etc.) — exatamente como qualquer link externo já funciona hoje. **Modelo novo** (migração `conteudo/0047`): `PaginaLivre` (titulo, slug único, `conteudo` rich text via `RichTextWidget`, ativa, ordem) — URL própria `/pagina/<slug>/`, **fora da árvore de `Categoria`** (não tem `categoria_pai`, não aparece em `mostrar_menu_superior`/`mostrar_navegue_area`/`mostrar_area_central`). `PaginaLivreBotao` (nome, `categoria` FK opcional — aponta para uma categoria JÁ EXISTENTE **sem mover/reparentar nada**, mesmo padrão não-destrutivo de `ColunaExtraBotao` — OU `link_externo`, ícone FA/imagem, `icone_largura`/`icone_altura` no mesmo padrão de tamanho de ícone já usado em todo o site desde a Parte 37) — e o botão também pode ser criado como **"botão completo do site"** (mesmo mecanismo já usado na Área do Site desde a Parte 20: sem escolher categoria existente + marcar o checkbox = cria uma `Categoria` RAIZ de verdade, que passa a existir em todas as árvores — Estrutura de Árvores, Painel Central, Organizador, Barra Superior — e pode ser editada normalmente por qualquer painel depois). **`Comentario` ganhou um TERCEIRO campo opcional** `pagina_livre` (FK, mutuamente exclusivo com `conteudo`/`categoria`, mesmo padrão dual já usado desde a Parte 33) — a property `origem` agora devolve `self.conteudo or self.categoria or self.pagina_livre`. View pública `pagina_livre_detalhe` (`conteudo/views.py`) e template `templates/pagina_livre_detalhe.html` reaproveitam **exatamente** a mesma seção de comentários (HTML/CSS/JS idênticos) já usada em `categoria.html`/`conteudo_detalhe.html` — moderação, respostas aninhadas, votos 👍/👎, tudo funciona igual, sem nenhum código novo de comentários. O grid de botões reaproveita a classe `.topic-grid`/`.topic-btn` já usada no índice geral "Documentos Curriculares". Painel admin (`conteudo/admin_views.py::paginas_livres_view`, template `templates/admin/paginas_livres.html`) segue exatamente o mesmo padrão de ações da Área do Site (criar/editar/excluir página, criar/excluir botão) — o select de "categoria já existente" usa `_arvore_flat_categorias()` + `data-pesquisavel` (mesma correção da Parte 46, já nascendo organizado). Nova permissão `conteudo.pode_acessar_paginas_livres` (mesmo padrão dos outros 9 painéis). Registrado também em Django Admin (`PaginaLivreAdmin` com inline de botões) como acesso alternativo/avançado. **Bug real encontrado e corrigido durante o teste**: a integração com o Microsoft Teams (`conteudo/teams_integration.py::_titulo_e_link`) assumia que todo comentário era `conteudo` OU `categoria` (código antigo: `if comentario.conteudo_id: ... else: comentario.categoria.nome`) — com o novo terceiro caso `pagina_livre`, essa suposição quebrava com `AttributeError: 'NoneType' object has no attribute 'nome'` sempre que um comentário de Página Livre tentava ser enviado ao Teams. Corrigido adicionando o terceiro `elif comentario.categoria_id:` explícito antes do `else` (que agora cobre só `pagina_livre`). Buscado no resto do código por essa mesma suposição perigosa (comentário sempre `conteudo` ou `categoria`) — nenhum outro lugar encontrado com o mesmo problema. **Testado ponta a ponta**: painel carrega 200 ✓, criar página funciona (URL própria gerada) ✓, página pública carrega com título/texto/comentários ✓, botão apontando para categoria existente não mexe na categoria original ✓, botão "completo do site" cria categoria raiz de verdade ✓, comentário pendente não aparece / aprovado aparece ✓, votação funciona (reaproveita o mesmo endpoint `/comentario/<pk>/votar/` sem nenhuma mudança, pois ele é agnóstico à origem do comentário) ✓, exclusão de página com cascade (botões + comentários) funciona ✓, regressão confirmada — home/categoria/busca continuam 200 sem nenhuma mudança. **Zero breaking changes**: nenhuma tela/view/template existente foi alterada além dos 3 pequenos ajustes no `ComentarioAdmin` (3 métodos ganharam um `elif`/`if` a mais para reconhecer a nova origem, sem remover nem alterar o comportamento dos casos já existentes) e da correção do bug real na integração Teams (que só cobre o caso novo, não muda o comportamento para comentários de Conteúdo/Categoria).
- **Leva anterior (2026-07-24 — "parte 46")**: **Painel "Adicionar Arquivos" — árvore de botões (Passo 1) trocada por select com busca instantânea**. O Dan reportou que a "barra de botões" do Passo 1 ("Escolha a categoria do site") estava "muito confusa" — o Passo 1 usava o widget `CategoriaPicker` (`conteudo/widgets.py`), que renderiza TODAS as 133 categorias do site como uma grade de botões com ícone, agrupados por categoria raiz e subgrupos, tudo expandido de uma vez, sem busca — uma parede grande de botões difícil de navegar. **Causa raiz**: esse widget é compartilhado por 3 formulários diferentes (`BannerAdminForm`, `ConteudoAdminForm` no Django Admin, e `adicionar_arquivos_view`) — mexer na classe `CategoriaPicker` diretamente quebraria os outros dois. **Solução aplicada só no painel "Adicionar Arquivos"** (sem tocar no widget `CategoriaPicker` em si, então Banner e Conteúdo no Django Admin continuam 100% iguais): trocado `{{ picker_html }}` (grade de botões) por um `<select name="categoria" data-pesquisavel="🔍 Pesquisar botão...">` — mesmo padrão já usado no Organizador ("Mover selecionados para") e no Painel Central ("Criar novo botão"), reaproveitando o helper já existente `_arvore_flat_categorias()` (de `admin_views.py`, criado na Parte 31) e o script `filtro_select.js` (já usado em 3+ lugares do projeto) — zero código novo de busca, só reaproveitamento do que já existia. O select mostra a árvore inteira indentada (`└` para subcategorias) e ganha uma caixinha de busca instantânea sem acento acima dele automaticamente. Ajustes no JS da página (`templates/admin/adicionar_arquivos.html`): a validação de submit e a função `duplicateItem()` usavam `document.querySelector('input[name="categoria"]:checked')` (sintaxe específica de radio button) — trocado para `document.querySelector('[name="categoria"]').value` (funciona com `<select>`); o listener genérico `if (e.target.name === 'categoria')` que dispara o carregamento de subcategorias **não precisou de nenhuma mudança** (dispara igual para `<select>` no evento `change`). Removido do `{% block extrahead %}` desta página o `<link>` para `admin_picker.css` (CSS do widget antigo, não usado mais aqui — Font Awesome CDN mantido, ainda usado pelos ícones de arquivo/formato da página). **Testado ponta a ponta**: painel carrega 200 com o select + busca funcionando (133 opções, indentação correta) ✓; fluxo completo de POST (criar conteúdo/link numa categoria escolhida pelo select) cria no lugar certo, idêntico a antes ✓; regressão confirmada — Banner e Conteúdo no Django Admin continuam mostrando a grade de botões `CategoriaPicker` original, sem nenhuma alteração ✓. **Zero breaking changes**: nenhuma mudança no backend (`request.POST.get('categoria')` já funcionava independente do tipo de widget), nenhuma migração necessária, nenhuma alteração no widget `CategoriaPicker` compartilhado.
- **Leva anterior (2026-07-24 — "parte 45")**: **Barra azul do título/texto da home ("home-intro") — correção do texto quebrando cedo + aparência configurável**. O Dan reportou que o texto "Referencial curricular da Educação Básica, elaborado em regime de colaboração entre Estado e municípios." quebrava em 2 linhas mesmo com bastante espaço vazio nas laterais. **Causa**: `.home-intro-texto` tinha `max-width: 700px` fixo no CSS, mesmo estando dentro de um `.container` que permite até 1200px — o texto nunca conseguia usar o espaço todo disponível. **Corrigido incondicionalmente** (efeito imediato, sem precisar mexer em nada no admin): trocado para `max-width: 100%` — agora o texto sempre usa o espaço inteiro do `.container` antes de quebrar linha. Testado: texto que antes quebrava em 2 linhas agora renderiza em 1 linha só (1121px de largura usada, antes limitado a 700px). Além da correção, o Dan pediu 4 recursos novos de personalização dessa barra, agora disponíveis em **Django Admin → Configuração do site → "🎨 Aparência da barra do título/texto da home"**: (1) **tamanho da letra** do título e do texto, separadamente (`home_intro_titulo_tamanho_fonte`/`home_intro_texto_tamanho_fonte`, px); (2) **largura máxima do conteúdo** (`home_intro_largura`, px) — controla até onde o título/texto podem esticar dentro da barra (que continua sempre ocupando 100% da tela — só o CONTEÚDO fica mais estreito se preenchido, decisão confirmada com o Dan antes de implementar); (3) **altura mínima da barra** (`home_intro_altura`, px); (4) **cor de fundo** (`home_intro_cor_fundo`) via seletor de cor nativo do navegador (`<input type="color">`, abre a paleta de cores do sistema operacional) e (5) **imagem de fundo** (`home_intro_imagem`, qualquer formato) — quando enviada, aparece atrás do título/texto com uma camada semi-transparente por cima (na cor de fundo escolhida, ou azul padrão `#1e3a5f` se nenhuma cor foi escolhida) para o texto continuar legível não importa a imagem. **Decisão de design confirmada com o Dan antes de implementar** (2 perguntas): (a) o recurso de "imagem/arquivos dentro da barra" é uma imagem decorativa de fundo — não uma lista de anexos para download; (b) "largura em px" controla o conteúdo dentro da barra, não o tamanho da própria barra (que segue sempre 100% da tela). Migração `conteudo/0046` (6 campos novos em `ConfiguracaoSite`, todos opcionais — `null=True, blank=True` ou `blank=True`). Novas properties (`home_intro_estilo_barra`, `home_intro_overlay_cor`, `home_intro_estilo_conteudo`, `home_intro_estilo_titulo`, `home_intro_estilo_texto`) montam os `style=` inline prontos — todas retornam string vazia quando os campos correspondentes estão vazios, preservando 100% a aparência de sempre. CSS novo em `static/css/style.css`: `.home-intro` ganhou `position:relative;overflow:hidden`, novas classes `.home-intro-bg` (camada da imagem, `position:absolute;inset:0`) e `.home-intro-overlay` (camada de cor semi-transparente, `opacity:.82`), e `.home-intro > .container` ganhou `position:relative;z-index:2` para o conteúdo ficar sempre por cima das camadas de fundo. Cache-busting do CSS incrementado para `?v=20260724-1`. **Testado ponta a ponta**: texto renderiza em 1 linha (fix confirmado via `getBoundingClientRect`) ✓, sem overflow horizontal no mobile (375px) ✓, os 6 campos aparecem no Django Admin com o seletor de cor nativo ✓, `largura=900px` aplica `max-width:900px` no `.container` interno (não na barra toda) ✓, `tamanho_fonte=30px` aplica no `<h1>` ✓, `cor_fundo` aplica como `background-color` (sem imagem) ou como camada `.home-intro-overlay` (com imagem) ✓, imagem de fundo renderiza como `.home-intro-bg` com a URL correta ✓. Todos os valores de teste foram limpos ao final (campos voltam a `None`/vazio, arquivo de imagem de teste removido do disco) — banco não ficou com nenhum dado de teste. **Zero breaking changes**: os 6 campos novos nascem vazios — sem o Dan preencher nada, a barra continua com a cor azul padrão, sem imagem, tamanho de fonte automático, e agora (efeito permanente da correção) sem quebrar o texto cedo.
- **Leva anterior (2026-07-24 — "parte 44")**: **`docker-compose.yml` — reinício automático dos containers + remoção do atributo `version` obsoleto**. O Dan notou que o Docker não estava reiniciando os containers sozinho quando o computador ligava/o Docker Desktop abria (ex.: depois de uma queda de energia ou reinício do Windows). Causa: nenhum dos 3 serviços (`db`, `web`, `teams_sync`) tinha política de `restart` configurada — o padrão do Docker é `no` (nunca reinicia sozinho). Também identificado (o próprio `docker compose` avisa isso no terminal) que a chave `version: '3.8'` no topo do arquivo está **obsoleta** desde a Compose Specification v2 — o Docker moderno ignora essa chave e já infere a versão sozinho. **Corrigido**: removida a linha `version: '3.8'` (e a linha em branco logo abaixo) do topo do arquivo; adicionado `restart: unless-stopped` aos 3 serviços (`db`, `web`, `teams_sync`) — com essa política, os containers voltam a subir sozinhos sempre que o Docker Desktop inicia (boot do Windows, ou o próprio Docker Desktop reiniciando), exceto se o Dan tiver parado o container manualmente de propósito (`docker compose stop`/`docker stop`), caso em que ele fica parado até ser religado manualmente — diferente de `restart: always`, que forçaria a subir mesmo depois de uma parada manual. Testado: `docker compose config` (comando de validação de sintaxe, não sobe containers) confirma os 3 serviços com `restart: unless-stopped` e nenhum aviso de `version` obsoleta. **Zero breaking changes**: nenhuma outra configuração do `docker-compose.yml` foi tocada (portas, volumes, variáveis de ambiente, comandos, healthcheck — tudo idêntico); a mudança só afeta o comportamento de reinício automático dos containers, não a aplicação em si.
- **Leva anterior (2026-07-24 — "parte 43")**: **Largura/Altura do ícone (px) nas 3 seções da home (Notícias/Destaques, Conteúdos recentes, Navegue por área)** — o Dan já tinha esse controle de tamanho de ícone em Categoria/ColunaExtra/ColunaExtraBotao (parte 37), mas faltava nas 3 seções fixas da home. Adicionados 6 novos campos em `ConfiguracaoSite` (migração `conteudo/0045`): `icone_destaques_largura`/`icone_destaques_altura`, `icone_recentes_largura`/`icone_recentes_altura`, `icone_areas_largura`/`icone_areas_altura` (todos `PositiveIntegerField`, px, opcionais) — mesmo padrão já usado em `Categoria.icone_largura`/`icone_altura`: se só a largura for preenchida, a altura assume o mesmo valor (ícone quadrado); `font-size` do glifo Font Awesome é escalado proporcionalmente também. Três novas properties (`icone_destaques_estilo_inline`, `icone_recentes_estilo_inline`, `icone_areas_estilo_inline`, via helper privado `_icone_secao_estilo()`) montam o `style="width:Npx;height:Npx;font-size:Npx;"` pronto. Campos adicionados ao `TituloSecoesForm` (`conteudo/forms.py`) com `NumberInput` estilizado (mesmo visual dos campos "Largura/Altura do ícone" já usados no formulário de coluna extra) e ao template `templates/admin/area_do_site.html` (2 inputs lado a lado, abaixo do upload de imagem, em cada um dos 3 blocos "Título e ícone de..."). Aplicado em `templates/home.html` nos 3 pontos onde o `<span class="title-icon">` de cada seção é renderizado (tanto para ícone Font Awesome quanto para imagem enviada). **Zero breaking changes**: campos novos nascem vazios (`None`) — sem o Dan preencher, os ícones das 3 seções continuam exatamente no tamanho padrão de sempre. Testado ponta a ponta via Django test client: painel exibe os 6 campos novos ✓, salvar com valores aplica o `style` correto no HTML renderizado da home ✓, campo vazio gera string vazia (sem `style=""` na tag) ✓, home renderiza sem erro com os valores em branco (estado padrão) ✓.
- **Leva anterior (2026-07-24 — "parte 42")**: **Botão "Currículo Atual" da home passou a obedecer o checkbox "Central?" da Barra Superior** — antes, a pílula "Currículo Atual" era renderizada de forma incondicional (hardcoded) em `home.html`, independente do campo `Categoria.mostrar_area_central` daquela categoria (usado por todos os outros botões raiz no mesmo lugar, painel `/admin/barra-superior/`). Corrigido: a view `home()` agora busca a categoria `curriculo-atual` e passa ao contexto (`categoria_curriculo_atual`); o template envolve a pílula num `{% if categoria_curriculo_atual.mostrar_area_central %}`. **Cuidado ao investigar**: o campo já existia (desde a parte 13) mas estava **desmarcado (`False`)** no banco no momento da mudança — para não alterar nada visualmente (pedido explícito do Dan), o campo foi marcado como `True` manualmente logo em seguida, deixando o botão aparecendo exatamente como antes. De agora em diante, desmarcar "Central?" na categoria "Currículo Atual" no painel Barra Superior faz o botão sumir da home; marcar, faz aparecer — igual a qualquer outro botão raiz. Testado: alternar entre `True`/`False` via shell confirma botão aparece/some corretamente no HTML renderizado; `manage.py check` limpo; confirmado visualmente no navegador. Zero migração necessária (campo já existia). Zero breaking changes.
- **Correção crítica (2026-07-23, consequência direta da Parte 41 — Lixeira)**: **`ATUALIZAR BANCO DOCKER.bat` falhava com `IntegrityError: ... violates foreign key constraint ... Key (categoria_id)=(191) is not present in table "conteudo_categoria"`** ao importar o dump no Postgres do Docker. Causa: o passo `[1/6]` do `.bat` roda `dumpdata` **sem a flag `--all`** — sem ela, o Django serializa usando o manager padrão (`objects`), que em `Categoria`/`Conteudo` **esconde tudo que está na lixeira** (Parte 41, `excluido_em__isnull=True`). Na prática: a categoria "Rotinas Pedagógicas - RPE - 2026" (pk 191) foi para a lixeira, mas um `Conteudo` que ainda apontava para ela (`categoria_id=191`) não estava na lixeira — o dump incluiu esse `Conteudo` (seu próprio `excluido_em` é nulo) mas **não incluiu a Categoria 191** (escondida pelo manager padrão), quebrando a chave estrangeira ao importar num Postgres vazio. **Corrigido**: adicionada a flag `--all` ao `dumpdata` em `BAT SEDU/ATUALIZAR BANCO DOCKER.bat` — com `--all`, o Django usa o `base_manager` de cada model, que em `Categoria`/`Conteudo` está configurado como `todos_objetos` (`Meta.base_manager_name`, Parte 41) e enxerga tudo, inclusive a lixeira. Testado: `dumpdata --all` local confirmado incluindo a categoria 191 (e demais itens na lixeira) no JSON exportado — resolve a violação de FK e mantém o Docker espelhando 100% do banco local (lixeira incluída, que é o objetivo do `.bat`). Ver armadilha #37 em `known_pitfalls.md`.
- **Leva mais recente (2026-07-23 — "parte 41")**: **Lixeira com recuperação em 30 dias (Categoria + Conteúdo) + recuperação emergencial de "Orientações Curriculares"** — o Dan excluiu por engano, no ambiente local, a categoria raiz "Orientações Curriculares" (18 subcategorias, 127 conteúdos) e pediu (1) recuperar imediatamente o que foi apagado e (2) um recurso permanente no painel admin para reverter exclusões em até 30 dias, com limpeza automática definitiva depois desse prazo.
  - **(1) Recuperação emergencial**: como o `db.sqlite3` local não tem histórico de versões, a recuperação usou um backup Docker/Postgres feito às 15:05 do mesmo dia (`docker_backups/backup_20260723_150450/banco_postgres.sql`), que ainda tinha a árvore intacta. Scripts one-off (`scripts_recuperacao/`) extraíram a categoria raiz (pk 64) + 19 subcategorias + 1 anexo do dump SQL e os recriaram no banco local com os MESMOS IDs; os 127 conteúdos **não haviam sido apagados de verdade** (regra do site: excluir categoria nunca apaga conteúdo, `Conteudo.categoria` é `SET_NULL`) — só ficaram órfãos, e foram reconectados ajustando apenas o campo `categoria_id` (nenhum outro dado do conteúdo foi tocado, preservando qualquer edição feita depois). Confirmado ponta a ponta: `/categoria/orientacoes-curriculares/` voltou a carregar (200), 18 subbotões visíveis, UTF-8 correto (ícone personalizado, descrição, tudo intacto).
  - **(2) Sistema de Lixeira** (permanente, para nunca mais precisar de recuperação manual): `Categoria` e `Conteudo` ganharam soft-delete via um novo campo `excluido_em` (migração `conteudo/0043`) + managers customizados (`CategoriaManager`/`ConteudoManager` como `.objects`, filtram automaticamente o que está na lixeira; `CategoriaTodosManager`/`ConteudoTodosManager` como `.todos_objetos`, enxergam tudo). **O ponto-chave do design**: `Model.delete()` e `QuerySet.delete()` foram sobrescritos para, em vez de apagar de verdade, só marcar `excluido_em=agora` — isso significa que **TODOS os botões de excluir que já existiam em TODO o site** (Django Admin — single e bulk —, Painel Central "Excluir botão selecionado"/Tela 2, Estrutura de Árvores, Organizador) **passaram a jogar para a lixeira automaticamente, sem precisar mudar o código de nenhum deles** — zero risco de quebrar alguma das ~10 telas que já excluíam categoria/conteúdo. Excluir uma categoria leva a subárvore inteira junto (recursivo) e NÃO desvincula mais os conteúdos dela (diferente do `SET_NULL` de antes) — eles ficam ligados e voltam automaticamente quando a categoria é restaurada, sem precisar reconectar manualmente como na recuperação emergencial de hoje. A ÚNICA exceção intencional: a ação "Excluir permanentemente" da Tela 2 do Painel Central continua sendo exclusão de verdade (`.hard_delete()`), já que o próprio nome promete isso. Nova permissão `conteudo.pode_acessar_lixeira` (mesmo padrão dos outros 8 painéis). Novo painel `/admin/lixeira/` (banner vermelho no dashboard) lista o que foi excluído (só o "topo" de cada subárvore, pra não poluir com filho-por-filho), com botões "Restaurar" e "Excluir já", e mostra quantos dias restam. **A limpeza automática dos 30 dias roda sozinha toda vez que a tela da Lixeira é aberta** (sem precisar agendar nada) — e também existe `python manage.py limpar_lixeira_expirada` / `BAT SEDU/LIMPAR LIXEIRA.bat` pra quem preferir agendar no Windows (mesmo padrão do `SINCRONIZAR TEAMS.bat`). **Testado ponta a ponta** (30+ cenários via Django test client): exclusão recursiva de subárvore ✓, conteúdo não fica mais órfão ✓, restaurar traz tudo de volta (categoria + subárvore + página do site volta a funcionar) ✓, exclusão via Estrutura de Árvores/Organizador/Painel Central/Django Admin (single e bulk) todas viram soft-delete automaticamente ✓, changelist do Django Admin não mostra itens da lixeira ✓, purga automática só remove itens com 30+ dias, item recente é preservado ✓, "Excluir permanentemente" da Tela 2 continua realmente permanente ✓. **Zero breaking changes** nas ~10 telas de exclusão existentes (nenhuma teve o código de exclusão alterado — só 3 mensagens de sucesso foram atualizadas de "removido permanentemente" para refletir que agora é recuperável, para não informar algo falso ao Dan).
- **Leva anterior (2026-07-23 — "parte 40" COMPLETA)**: **Ação "Mover" no Django Admin (Conteúdo e Categoria) + Balãozinho de Comentários Pendentes no Menu Lateral Esquerdo** — o Dan pediu 2 implementações: 
  - **(1) Balãozinho de comentários no menu lateral** — investigado o "contador não apareceu" e confirmado: o contador do dashboard principal (`/admin/`, painel lateral direito, ao lado de "Ações recentes") já funcionava normalmente, mas Dan queria também **junto ao item "Comentários" no menu de navegação lateral esquerdo** (o menu fixo com busca, presente em TODA página do admin). **Implementado**: novo `templates/admin/app_list.html` (override do template padrão do Django, cópia fiel + 1 bloco condicional) — quando o loop chega no model "Comentário", insere um badge azul-ciano pequeno (pílula, mesma cor do dashboard) com contagem ao vivo (reaproveita `{% comentarios_pendentes_count %}` da parte 38) — só aparece com 1+ pendentes, link direto para lista filtrada. Como `app_list.html` é incluído em TODA página do admin (não só dashboard), o balão agora aparece sempre. **Testado**: 6 páginas diferentes (`/admin/`, `/admin/conteudo/`, listagens de Categoria/Conteúdo/Comentário/Banner) — badge presente/correto em todas, desaparece quando chega a zero pendentes ✓. **Zero breaking changes** — resto é cópia exata do padrão Django, só foi inserido o bloco do badge.
  - **(2) Ação "📂 Mover selecionados"** — adicionada ao dropdown "Ação" em **Conteúdo** e **Categoria** (mesmo menu que "Publicar/Arquivar/Remover" em Conteúdo, "Ativar/Desativar/Remover" em Categoria). Ao marcar itens e escolher, abre tela intermediária (padrão "Responder em massa", parte 34) com lista dos itens + select com busca instantânea listando TODA a árvore, indentada. Em **Conteúdo** (`ConteudoAdmin.mover_selecionados` → `mover/`, template `conteudo_mover_massa.html`): escolhe destino, campo `categoria` é atualizado. Em **Categoria** (`mover_selecionadas` → `mover/`, template `categoria_mover_massa.html`): escolhe novo pai ou "vira raiz", campo `categoria_pai` é atualizado — com **proteção contra ciclo** (reaproveita `_eh_descendente()` de `arvore_views.py`): lista esconde automaticamente os próprios botões selecionados e qualquer botão dentro deles; se ciclo for tentado, é bloqueado com aviso. Reaproveita `_arvore_flat_categorias()` de `admin_views.py` (parte 31, Organizador) — zero duplicação. **Zero migração necessária** (usa campos `categoria`/`categoria_pai` já existentes). **Testado**: mover conteúdo ✓, mover categoria ✓, proteção ciclo ✓, ações antigas intactas ✓. **Zero breaking changes** — só acréscimo de uma ação nova em cada admin.
- **Leva anterior (2026-07-23 — "parte 39")**: **Integração de Comentários com o Microsoft Teams (grupo GECEB)** — base completa implementada, a pedido do Dan (documento `teams automacao comentarios.md`), para que todo comentário feito no site seja enviado automaticamente para um canal do Teams e, quando a equipe responder na conversa, a resposta volte sozinha para o site. **Pesquisa técnica importante**: confirmado (documentação Microsoft + comunidade Power Automate) que o conector do Teams **não dispara automaticamente para respostas dentro de uma conversa** (só para mensagem nova na raiz do canal) — não existe atalho de baixo código para isso. Por isso o desenho final é: **(A) envio site→Teams** via webhook do Power Automate (modelo pronto "Publicar em um canal quando um webhook for recebido", usando o app Workflows que o Dan já tem instalado) — função `enviar_comentario_para_teams()` em `conteudo/teams_integration.py`, chamada logo após `Comentario.objects.create(...)` nas 2 views que criam comentários (`categoria_detalhe`/`conteudo_detalhe` em `conteudo/views.py`), sempre best-effort (nunca derruba o site se o Teams estiver fora ou não configurado); **(B) leitura de respostas Teams→site** via **consulta periódica à Microsoft Graph API** (não webhook de entrada) — novo comando `python manage.py sincronizar_teams` (`conteudo/management/commands/sincronizar_teams.py`) que (1) vincula mensagens recém-postadas no canal ao comentário certo (procura o marcador discreto "ID do comentário: N" no corpo da mensagem) e (2) verifica se chegou resposta nova da equipe, validando o remetente contra uma lista opcional de e-mails autorizados (`TEAMS_MEMBROS_AUTORIZADOS`) — quando encontra resposta, grava nos MESMOS campos `Comentario.resposta`/`data_resposta` que o admin já usa manualmente, então a resposta aparece no site exatamente como uma resposta manual apareceria. Esse desenho elimina qualquer endpoint de entrada exposto publicamente (o Django só faz chamadas PARA FORA, nunca recebe chamadas do Teams/Power Automate) — funciona igual local, Docker, homologação e produção, sem precisar de túnel/ngrok para essa parte. Novos campos em `Comentario` (migração `conteudo/0042`): `teams_status` (não_enviado/enviado/vinculado/respondido/erro), `teams_message_id`, `teams_data_envio`, `teams_ultima_verificacao` — todos opcionais, zero impacto em comentários existentes. Novas variáveis de ambiente em `settings.py` (`TEAMS_WEBHOOK_URL`, `TEAMS_CLIENT_ID/SECRET`, `TEAMS_TENANT_ID`, `TEAMS_TEAM_ID`, `TEAMS_CHANNEL_ID`, `TEAMS_MEMBROS_AUTORIZADOS`) — todas vazias por padrão, integração inteira é no-op seguro sem elas (testado: criar comentário sem nenhuma configurada não gera erro, `sincronizar_teams` sem credenciais só avisa e sai). **Os 3 domínios do projeto** (oficial `curriculo.sedu.es.gov.br`, homologação `curriculohm.sedu.es.gov.br` e `curriculodev.sedu.es.gov.br`) ficam hardcoded uma única vez em `DOMINIOS_CONHECIDOS` (`settings.py`), incluídos automaticamente em `CSRF_TRUSTED_ORIGINS` — o código nunca precisa ser editado na troca de ambiente; qualquer link gerado dinamicamente usa `request.build_absolute_uri()`, então se ajusta sozinho ao domínio de onde o site está rodando no momento. Admin (`ComentarioAdmin`): novo badge `teams_status_badge` na lista (cinza/azul/ciano/verde/vermelho conforme o status) + nova seção colapsável "🔗 Integração com Microsoft Teams (GECEB)" com os 4 campos novos (somente leitura, controlados pela integração). **Solução provisória enquanto a TI da SEDU não libera o cadastro no Azure AD** (necessário só para a Parte B, leitura de respostas): o Dan/GECEB cola a resposta lida no Teams diretamente no campo "Resposta do administrador" já existente — zero código novo, aparece no site do mesmo jeito. Docker: `requirements.txt` ganhou `requests==2.32.3`; `docker-compose.yml` ganhou as 7 variáveis `TEAMS_*` no serviço `web` + novo serviço leve `teams_sync` (mesma imagem, sem porta exposta, loop `sincronizar_teams` a cada 180s, não precisa de Celery/Redis). Novo `BAT SEDU/SINCRONIZAR TEAMS.bat` para rodar/agendar localmente no Windows. Novo documento `GUIA_CONFIGURACAO_TEAMS.md` na raiz do projeto com o passo a passo completo para o Dan configurar o Power Automate (Parte A, disponível hoje) e o que pedir à TI da SEDU + o que fazer depois (Parte B). **Testado ponta a ponta**: `python manage.py check` limpo, migração `0042` aplicada sem erro, criar comentário via shell sem nenhuma variável Teams configurada não gera exceção (`teams_status` fica `nao_enviado`), `sincronizar_teams` sem credenciais avisa e sai normalmente, lista de comentários no admin carrega (200) com o badge novo. **Zero breaking changes** — nenhuma view, template, admin ou lógica existente foi alterada além de 2 linhas novas (chamada best-effort) nas 2 views de criação de comentário; tudo mais é acréscimo.
- **Leva anterior (2026-07-23 — "parte 38")**: **Contador de comentários pendentes no painel administrativo** — o Dan pediu um contador discreto e futurista, no lado direito do admin (mesmo painel onde já aparecia "Ações recentes"), que mostra quantos comentários estão aguardando moderação, e que "volta a zero" à medida que ele aprova/recusa/exclui os comentários. Implementado como um módulo novo no `{% block sidebar %}` de `templates/admin/index.html` (dashboard do admin) — card escuro com gradiente, glow ciano, número grande, e um ponto pulsante (CSS `@keyframes`) que só anima quando há pendentes; quando chega a zero, mostra "✓ Tudo revisado" em verde. O card inteiro é um link direto para `/admin/conteudo/comentario/?status__exact=pendente` (lista já filtrada). Novo template tag `conteudo/templatetags/comentarios_extras.py` (`{% comentarios_pendentes_count %}`) faz uma contagem **ao vivo** (`Comentario.objects.filter(status='pendente').count()`) a cada carregamento do dashboard — não precisa de nenhuma lógica extra para "zerar": como é sempre uma consulta ao banco no momento do carregamento da página, o número reflete automaticamente o estado atual assim que o Dan aprova/recusa/exclui um comentário (nas ações em lote já existentes do `ComentarioAdmin`, inalteradas) e volta ao dashboard. Só aparece para quem tem a permissão `conteudo.view_comentario` (nativa do Django, ninguém perde nem ganha acesso). **Testado ponta a ponta no navegador**: criados 2 comentários de teste com `status='pendente'` → contador subiu de 0 para 2 ✓; aprovado 1 via ação "✅ Aprovar e publicar" → contador caiu para 1 ✓; excluído o restante → contador voltou a 0 com "✓ Tudo revisado" ✓; dados de teste (comentários e usuário temporário criado só para o teste visual) removidos ao final, banco limpo. **Zero breaking changes** — nenhuma view, template ou lógica existente foi alterada; só uma nova seção adicionada ao bloco sidebar (que já existia, mostrando "Ações recentes") e 2 arquivos novos (`__init__.py` + `comentarios_extras.py` em `conteudo/templatetags/`, pasta nova no app). Nenhuma migração de banco necessária.
- **Leva anterior (2026-07-22 — "parte 37" + extensão)**: **2 pedidos do Dan resolvidos + extensão do 2º pedido** — (1) **Sync de mídia Docker corrigido**: `ATUALIZAR BANCO DOCKER.bat` só sincronizava o BANCO (dumpdata/loaddata) — os ARQUIVOS de mídia (ícones, imagens, banners, anexos) nunca eram copiados, porque `docker-compose.yml` usa um volume Docker separado (`media_data:/app/media`) para a mídia dentro do container, isolado da pasta `media\` local. Resultado: ícone enviado localmente aparecia quebrado no ambiente Docker depois do sync, porque o banco (com a referência ao arquivo) ia, mas o arquivo em si, não. Corrigido: novo passo `[3/6]` no `.bat` que roda `docker compose cp media\. web:/app/media/` — copia (sem apagar nada já existente no Docker) todo o conteúdo da pasta `media\` local para dentro do container antes de aplicar as migrações/importar o banco. Não mexe no `docker-compose.yml` nem em nada mais — só adiciona esse passo de cópia. (2) **Tamanho do ícone dos botões configurável**: novos campos `Categoria.icone_largura`/`icone_altura` (px, opcionais, migração `conteudo/0039`) — quando preenchidos, controlam o tamanho do ícone daquele botão específico (Font Awesome ou imagem), sobrescrevendo o tamanho padrão do CSS via `style` inline (property `icone_estilo_inline`). Adicionado em **5 pontos administrativos**: Django Admin (fieldset "Aparência" de Categoria), Painel Central (Criar novo botão, Criar subárea, Editar botão selecionado), Organizador (Criar novo botão dentro de X), Estrutura de Árvores (modal Criar botão + seção "Ícone" do painel de edição), Área do Site (botão completo do site). Aplicado nos templates onde o ícone de um botão aparece: "Navegue por área" e área central da home, índice geral (`topic-icon`), subbotões (chips e cards), "Conteúdos recentes" (quando um botão aparece lá) e busca. **Extensão (mesmo dia)**: o Dan pediu para incluir também os botões de **"colunas extras"** (painel Área do Site) — adicionados os MESMOS campos `icone_largura`/`icone_altura` (+ property `icone_estilo_inline`) ao modelo `ColunaExtraBotao` (migração `conteudo/0040`), campos sempre visíveis no formulário "Adicionar botão" (antes ficavam escondidos dentro do bloco "criar como botão completo do site" — corrigido, já que valem para qualquer botão de coluna extra, com ou sem esse checkbox), e novo botão "🔍 Ajustar tamanho do ícone" em cada botão já criado na lista (abre um mini-formulário inline com largura/altura + "Salvar tamanho", nova action `editar_botao_coluna`). Aplicado nos 2 pontos de `home.html` onde `coluna.botoes.all` é renderizado (colunas à esquerda e à direita). **2ª extensão (mesmo dia)**: o Dan pediu o mesmo controle para o **ícone do TÍTULO da coluna** (o ícone ao lado do nome da coluna, ex.: "🌟 Editais") — mesmos 2 campos `icone_largura`/`icone_altura` + `icone_estilo_inline` adicionados ao modelo `ColunaExtra` (migração `conteudo/0041`), campos no formulário "Criar nova coluna" e no formulário de edição de cada coluna existente (`templates/admin/area_do_site.html`), aplicado nos 2 pontos de `home.html` onde o `<h2 class="section-title">` de cada coluna renderiza o ícone (`.title-icon`). Testado ponta a ponta via Django test client nos 3 níveis (botão de Categoria, botão de coluna extra, título de coluna extra): criar/editar com tamanho, confirmado no HTML renderizado da home em cada caso. Zero breaking changes — campos vazios (padrão) mantêm o tamanho de ícone exatamente como sempre foi, nos 3 níveis.
...
- **Correção crítica (2026-07-22, após a parte 36 — sync Docker/Postgres)**: **"ATUALIZAR BANCO DOCKER.bat" falhava com `DataError: value too long for type character varying(200)`** ao importar para o Postgres. Causa: vários campos `URLField` do projeto (`Conteudo.url_video`/`url_externa`, `Banner`/`Cartaz`/`CarrosselImagem.link`/`url_imagem`, `RodapeImagem.url`, `ColunaExtraBotao.link_externo`, e os já parcialmente ajustados `Categoria.url_externa`/`Anexo.url`) não tinham `max_length` explícito — o Django usa 200 por padrão. **SQLite nunca aplicou esse limite** (por isso funcionava local sem erro), mas o **Postgres aplica rigorosamente** `varchar(200)`, e um link real do SharePoint (219 caracteres, no conteúdo "PJS 2026 Slides") estourava esse limite na hora do `loaddata`. Corrigido: todos os 12 campos `URLField` do projeto (`conteudo/models.py`) agora usam `max_length=1000` — migração **`conteudo/0038`** (só altera o tamanho da coluna, nenhum dado perdido/truncado). Confirmado que nenhum dado existente ultrapassa 1000 caracteres. Depois desta migração, `ATUALIZAR BANCO DOCKER.bat` deve rodar sem esse erro (o Dan precisa rodar `python manage.py migrate` local antes, ou o próprio `.bat` já roda migrate dentro do container).
- **Correção final (2026-07-22, após a parte 36)**: **rótulo "Botão" removido de "Conteúdos recentes" da home** — quando um botão (categoria) aparecia na seção "Conteúdos recentes", era exibido com o rótulo "Botão" embaixo do nome (template `home.html`, linha 239). Removido: agora mostra apenas o nome do botão, sem rótulo adicional (consistente com o design dos cards de conteúdo e do rest do site).
- **Correção crítica (2026-07-22, após a parte 36)**: **botão criado com URL abria uma página intermediária em vez de ir direto ao link.** Causa: ao criar um botão preenchendo só o campo "URL / Link" (não "Mais links"), 4 dos 5 pontos administrativos criavam um `Conteudo(tipo='link')` FILHO da categoria em vez de (ou além de) preencher `Categoria.url_externa` — o campo que faz o botão redirecionar direto (property `link_externo`, já usado em todos os templates desde muito antes). Resultado: clicar no botão abria a página da categoria, que por sua vez mostrava mais um card/botão para clicar — o "botão dentro do botão" relatado pelo Dan (exemplo: "Classssificação Jovem Senador 2026"). **Corrigido nos 5 pontos**: `conteudo/arvore_views.py` (`_api_criar`, Estrutura de Árvores) agora seta `url_externa` na criação; `painel/views.py` (`_criar_no`, `_criar_subareas`) e `conteudo/admin_views.py` (`criar_subcategoria`, `criar_botao` da Área do Site) removeram a criação do `Conteudo` filho redundante (já setavam `url_externa` corretamente, só sobrava o filho morto). **Retroativo**: identificadas e corrigidas **32 categorias já existentes** com o mesmo padrão (a maioria migrada do WordPress antigo) — script one-off setou `url_externa` a partir do único conteúdo-link filho de cada uma (critério seguro: categoria sem subcategoria, sem anexo, com exatamente 1 conteúdo tipo link cujo título é igual ao nome do botão). Testado ponta a ponta nos 5 pontos via Django test client (nenhum cria mais conteúdo filho, `url_externa` sempre preenchido) e confirmado no HTML renderizado que o `href` do botão aponta direto pro link externo. Campo "Editar botão selecionado" do Painel Central não foi afetado (já tinha dois campos DISTINTOS e corretamente rotulados: `editar_url_externa` para redirecionamento direto e `editar_url` — rotulado "cria um conteúdo tipo link dentro do botão" — para quem quer isso de propósito).
- **Leva mais recente (2026-07-22 — "parte 36")**: **Anexo de LINK/URL em todos os painéis administrativos** — o `Anexo` (modelo já usado para PDFs/arquivos) ganhou um campo opcional `url` (migração `conteudo/0037`, `arquivo` também virou opcional) — quando preenchido, o anexo aparece na mesma seção "Arquivos para download" (categoria e conteúdo) com o MESMO visual dos anexos de arquivo, só que com ícone de link (`fa-link`), badge "LINK" (cor ciano) e abre a URL em nova aba em vez de baixar. Permite anexar uma URL com um nome amigável sem precisar criar um botão/card clicável para ela — alternativa mais discreta ao recurso "Mais links/URLs" já existente (que cria um `Conteudo` tipo link, renderizado como card). Adicionado em **5 pontos administrativos** (os mesmos já tocados nas partes 22-25 de drag-and-drop/múltiplos links): Painel Central (4 sub-pontos: Criar novo botão, Criar subárea, Editar botão selecionado, Anexos do conteúdo), Organizador (2: Criar novo botão dentro de X, Adicionar novo arquivo/URL a X), Estrutura de Árvores (2: modal Criar novo botão, painel de detalhes de qualquer nó), Área do Site (1: botão completo do site). Backend: helper `_criar_anexos_link(post, prefix, conteudo=None, categoria=None)` duplicado em `painel/views.py` e `conteudo/admin_views.py` (mesmo padrão de `_criar_links_extra`, mas cria `Anexo(url=...)` em vez de `Conteudo(tipo='link')`), e nova action AJAX `associar_anexo_link` em `conteudo/arvore_views.py`. Frontend: mesma UI de "linhas dinâmicas Nome+URL" já usada para "Mais links", reaproveitando os helpers JS existentes (`pcNovaLinhaLink`, `orgNovaLinhaLink`, `asNovaLinhaLink`, `EA._novaLinhaLink`) com um parâmetro extra `campo='anexolink'` que troca o sufixo dos nomes de campo (`{prefix}_anexolink_nome`/`{prefix}_anexolink_url`) sem alterar o comportamento dos campos antigos. Zero breaking changes — campo `url` sempre vazio em anexos existentes, tudo continua igual. Testado ponta a ponta via Django test client nos 5 pontos: Painel Central (criar_no), Organizador (criar_conteudo_aqui), Estrutura de Árvores (criar + associar_anexo_link), Área do Site (criar_botao) — todos criando `Anexo(url=..., eh_link=True, extensao='LINK')` corretamente. Detalhes no histórico item 49.
- **Leva anterior (2026-07-22 — "parte 35")**: **Barra de formatação completa em todos os campos de texto do site** — `RichTextWidget` (usado em Área do Site, Editor do Rodapé, Organizador, Painel Central, etc.) ganhou 2 novos botões: **cor do texto** e **cor de destaque (marca-texto)**, ambos com seletor de cor nativo. Campo "Descrição" da Estrutura de Árvores agora usa o **mesmo editor completo** em vez de textarea simples — colar de qualquer fonte **preserva formatação** automaticamente (comportamento nativo de contenteditable). Testado ponta a ponta: negrito + cor vermelha + destaque verde + sincronização com campos de salvamento funcionando, mesmo quando seletor de cor tira foco temporariamente (seleção restaurada antes de aplicar). Zero breaking changes — apenas expansão de funcionalidade. Detalhes no histórico item 48.
- **Leva anterior (2026-07-22 — "parte 34" + ajuste fine-tuning)**: **4 correções de UX e bugs** — (1) **Selects cortando texto**: altura fixa 38px aplicada em 4 painéis (Editor do Rodapé, Área do Site, Organizador, Estrutura de Árvores) — texto em caixas de seleção agora visível por completo, sem truncamento. (2) **Títulos de colunas extras com formatação rich text**: campo `ColunaExtra.titulo` mudou de `CharField` para `TextField`, editor `RichTextWidget` injetado diretamente na view `area_do_site_view` — suporta negrito/itálico/alinhamento/listas; (3) **Resposta em massa para comentários**: nova ação `responder_em_massa` no `ComentarioAdmin` (além de aprovar/recusar/excluir em massa já existentes) — redireciona para tela intermediária (`templates/admin/comentario_responder_massa.html`), textarea de resposta, aplicada simultaneamente a todos os comentários selecionados via `queryset.update(resposta=..., data_resposta=timezone.now())`; (4) **Vídeo sem iframe (elimina erro 153 do YouTube)**: removido `<iframe srcdoc>` que quebrava quando dono do vídeo desabilitava embedding — agora SEMPRE renderiza botão `<a target="_blank" href=...>Assistir vídeo</a>` que abre URL em nova aba, elimina o erro de vez para qualquer vídeo (YouTube com/sem embedding bloqueado, Vimeo, ou link genérico); property `video_embed_valido` e método `get_video_embed_url()` continuam existindo no models.py (regex robusto mantido, inofensivo, base se precisar reativar embed no futuro). Migração `conteudo/0036` (CharField→TextField em ColunaExtra). **Fine-tuning 2026-07-22**: **Layout das colunas extras idêntico ao de "Navegue por área"** — regra CSS `.home-coluna-extra .areas-list` mudou de 1-coluna (layout de lista) para 2-coluna (layout de grade com botões quadrados), ícone em cima + texto embaixo, mesmo padding/tamanho das seções principais, apenas 2 colunas em vez de 3 por ser espaço mais estreito. **Zero breaking changes** — apenas melhorias de UX e resolução de bugs reportados pelo Dan. Testado ponta a ponta: 4 painéis carregando 200, editor rich text renderizando, respostas aplicadas a múltiplos comentários, vídeo sem iframe funcionando para YouTube/genérico, botões de coluna extra com layout idêntico a "Navegue por área". Detalhes no histórico item 47.
...
- **Migrações aplicadas**: `conteudo/0012-0047` + `painel/0002-0003` + `inteligencia/0002`. **Cronologia das mais recentes (Parte 47 + Parte 45 + Parte 43 + Parte 41 + Parte 39 + Parte 37 + Parte 36 + correções)**: 
  - **`conteudo/0047`** (2026-07-24, Parte 47): novos modelos `PaginaLivre`/`PaginaLivreBotao` + `Comentario.pagina_livre` (FK opcional) + permissão `pode_acessar_paginas_livres` — painel "Páginas Livres"
  - **`conteudo/0046`** (2026-07-24, Parte 45): `ConfiguracaoSite.home_intro_titulo_tamanho_fonte/texto_tamanho_fonte/largura/altura/cor_fundo/imagem` — aparência configurável da barra azul do título/texto da home
  - **`conteudo/0045`** (2026-07-24, Parte 43): `ConfiguracaoSite.icone_destaques_largura/altura`, `icone_recentes_largura/altura`, `icone_areas_largura/altura` — controle de tamanho do ícone das 3 seções da home
  - **`conteudo/0044`** (2026-07-23, Parte 41): `ConfiguracaoSite` ganha a permissão `pode_acessar_lixeira` (só `AlterModelOptions`, nenhuma coluna nova)
  - **`conteudo/0043`** (2026-07-23, Parte 41): `Categoria.excluido_em`/`Conteudo.excluido_em` — sistema de Lixeira (soft-delete, recuperação em 30 dias)
  - **`conteudo/0042`** (2026-07-23, Parte 39): `Comentario.teams_status`/`teams_message_id`/`teams_data_envio`/`teams_ultima_verificacao` — integração de comentários com Microsoft Teams (GECEB)
  - **`conteudo/0041`** (2026-07-22, Parte 37 ext.2): `ColunaExtra.icone_largura`/`icone_altura` — controle de tamanho do ícone do TÍTULO da coluna extra
  - **`conteudo/0040`** (2026-07-22, Parte 37 ext.1): `ColunaExtraBotao.icone_largura`/`icone_altura` — controle de tamanho do ícone dos botões DENTRO da coluna extra
  - **`conteudo/0039`** (2026-07-22, Parte 37): `Categoria.icone_largura`/`icone_altura` — controle de tamanho do ícone dos botões normais (5 painéis administrativos)
  - **`conteudo/0038`** (2026-07-22, após Parte 36): todos os 12 campos `URLField` do projeto passaram de 200 para `max_length=1000` — corrige `DataError: value too long for type character varying(200)` ao sincronizar SQLite local para Postgres/Docker (links longos como SharePoint)
  - **`conteudo/0037`** (2026-07-22, Parte 36): `Anexo.url` novo campo opcional + `arquivo`/`nome` viram opcionais — anexos de link/URL (ícone fa-link, badge LINK ciano, abre em nova aba)
- **Arquivos Docker novos**: `Dockerfile` (Python 3.12 slim + dependências de sistema), `docker-compose.yml` (db: Postgres 16 + volumes, web: Django com migrations automáticas), `BAT SEDU/ATUALIZAR BANCO DOCKER.bat` (sincronização local→Docker em um clique, **desde a parte 37 também copia a pasta `media\` local para dentro do container** — antes só sincronizava o banco).
...
conteudo/                # App principal do site
  ...
  templatetags/          # comentarios_extras.py: {% comentarios_pendentes_count %} (parte 38, contador do dashboard admin)
  teams_integration.py   # Integração de Comentários com Microsoft Teams/GECEB (parte 39):
                         #   enviar_comentario_para_teams, obter_token_graph,
                         #   vincular_mensagens_pendentes, verificar_respostas
  migrations/            # 0001 inicial → 0042 (Comentario.teams_*, parte 39)
  management/commands/   # ver seção "Management commands" (+ sincronizar_teams, parte 39)
...
- **`icone_largura`/`icone_altura`** (PositiveIntegerField, px, opcionais, migração 0039 — parte 37, 2026-07-22) — controla o TAMANHO do ícone daquele botão específico (Font Awesome ou imagem), sobrescrevendo o tamanho padrão do CSS. Property **`icone_estilo_inline`** monta o `style="width:Npx;height:Npx;font-size:Npx;"` pronto (só largura preenchida = ícone quadrado; font-size calculado proporcionalmente para o glifo Font Awesome também escalar). Vazio (padrão) = tamanho normal do site, sem nenhuma mudança visual. Editável em **5 pontos administrativos**: Django Admin (fieldset "Aparência"), Painel Central (Criar novo botão, Criar subárea, Editar botão selecionado), Organizador (Criar novo botão dentro de X), Estrutura de Árvores (modal Criar botão + seção "Ícone" do painel de edição), Área do Site (botão completo do site). Aplicado nos templates onde o ícone aparece: "Navegue por área", área central, "Conteúdos recentes", índice geral, subbotões (chips e cards), busca.
- **`ColunaExtraBotao.icone_largura`/`icone_altura`** (migração `conteudo/0040`, Parte 37 ext.1, 2026-07-22) — mesmos 2 campos + property para os botões DENTRO de uma coluna extra, sempre visíveis no formulário "Adicionar botão", novo botão "🔍" em cada botão já criado abre mini-formulário inline (action `editar_botao_coluna`). Editável no formulário e via ação.
- **`ColunaExtra.icone_largura`/`icone_altura`** (migração `conteudo/0041`, Parte 37 ext.2, 2026-07-22) — mesmos 2 campos + property para o ícone do TÍTULO da coluna extra (o ícone ao lado do nome, ex.: "🌟 Editais"). Editável nos formulários "Criar nova coluna" e "Editar coluna".
...
- **`excluido_em`** (DateTimeField, null, migração 0043 — parte 41, 2026-07-23) — **Lixeira**: preenchido automaticamente quando o botão é excluído (por qualquer painel), em vez de apagar de verdade. `Categoria.objects` (manager padrão) sempre filtra `excluido_em__isnull=True` — nunca mostra o que está na lixeira, em NENHUM lugar do site/admin, sem precisar mudar código nenhum. `Categoria.todos_objetos` enxerga tudo (usado só pela tela `/admin/lixeira/`). `Categoria.delete()` foi sobrescrito para marcar `excluido_em` (recursivamente, para toda a subárvore) em vez de apagar — `Categoria.hard_delete()` faz a exclusão de verdade. Método `restaurar()` desfaz (categoria + subárvore inteira de uma vez). Ver seção "Lixeira" mais abaixo.
...
- `categoria` (FK, **SET_NULL**) — vínculo primário; o Painel Central soma outros locais via `Vinculo`. ⚠️ SET_NULL só dispara numa exclusão REAL (`hard_delete()`) — quando a categoria vai para a lixeira (soft-delete padrão), o conteúdo continua ligado a ela e volta a aparecer sozinho se a categoria for restaurada
- **`excluido_em`** (DateTimeField, null, migração 0043 — parte 41, 2026-07-23) — mesmo mecanismo de Lixeira da Categoria (`Conteudo.objects`/`Conteudo.todos_objetos`, `delete()`/`hard_delete()`/`restaurar()`). Única exceção no site: a ação "Excluir permanentemente" da Tela 2 do Painel Central (`painel/views.py`) chama `hard_delete()` de propósito, pulando a lixeira — o nome da ação já promete isso
...
- `arquivo` (upload_to `anexos/%Y/%m/`, **opcional desde a migração 0037**), **`url`** (URLField, opcional, migração 0037 — parte 36), `nome` (opcional, agora "Nome de exibição"), `ordem`
- **Anexo de link (parte 36, 2026-07-22)**: preencher `url` (deixando `arquivo` vazio) faz o anexo virar um LINK em vez de um arquivo — aparece na mesma seção "Arquivos para download" com o mesmo visual, mas com ícone `fa-link`, badge "LINK" (ciano) e abre a URL numa aba nova em vez de baixar. Preencher só UM dos dois (`arquivo` OU `url`), nunca os dois.
- Propriedades: `extensao` (retorna `'LINK'` quando é um anexo de link), `nome_exibicao` (usa `nome`, senão nome do arquivo, senão a própria URL), **`eh_link`** (bool — `True` quando `url` preenchida e `arquivo` vazio), **`link_href`** (URL de destino — `url` ou `arquivo.url`, os templates SEMPRE usam essa property em vez de `arquivo.url` diretamente)
- Inlines no admin de Conteudo e de Categoria (3 linhas extras, campo `url` visível ao lado de `arquivo`)
- No site: seção "Arquivos para download" na página da categoria e do conteúdo — cards verticais com ícone colorido por tipo (PDF vermelho, Word azul, Excel verde, PPT laranja, vídeo roxo, imagem ciano, **link ciano** desde a parte 36)
- **Onde anexar um link (5 pontos administrativos, parte 36)**: Painel Central (Criar novo botão, Criar subárea, Editar botão selecionado, Anexos do conteúdo), Organizador (Criar novo botão, Adicionar novo arquivo/URL), Estrutura de Árvores (modal Criar botão, painel de detalhes de qualquer nó), Área do Site (botão completo do site) — mesmo padrão de "linhas dinâmicas Nome+URL" já usado para "Mais links/URLs", só que cria `Anexo(url=...)` em vez de `Conteudo(tipo='link')`. Diferença de uso: "Mais links" cria um BOTÃO/card clicável cheio; "Anexar link" cria uma entrada discreta na lista de anexos/downloads.
- **`eh_imagem`** (property, parte 49, 2026-07-24): `True` quando o anexo é um ARQUIVO de imagem enviado (jpg/jpeg/png/gif/webp/bmp/svg — anexos de link nunca contam). Anexos de imagem em **Categoria** NÃO aparecem mais em "Arquivos para download" — a view `categoria_detalhe` os separa em `anexos_imagens` (contexto), renderizados como galeria visual (`.anexo-imagens-grid`, grid lado a lado, clicável para abrir em tamanho real) na área de conteúdo do botão, no lugar de "Nenhum conteúdo nesta categoria ainda" quando vazia. Anexos de imagem em **Conteudo** (não Categoria) continuam inalterados, ainda aparecem em "Arquivos para download" — a mudança foi escopada só para `categoria.html`.
...
- **Ícones das 3 seções da home** (`icone_destaques`/`icone_recentes`/`icone_areas` + variantes `_imagem`, parte 18) — desde a **parte 43** (migração `conteudo/0045`), cada um também tem **`_largura`/`_altura`** (px, opcionais, mesmo padrão de `Categoria.icone_largura`/`icone_altura`): `icone_destaques_largura`/`icone_destaques_altura`, `icone_recentes_largura`/`icone_recentes_altura`, `icone_areas_largura`/`icone_areas_altura`. Properties `icone_destaques_estilo_inline`/`icone_recentes_estilo_inline`/`icone_areas_estilo_inline` (via helper privado `_icone_secao_estilo()`) montam o `style="width:Npx;height:Npx;font-size:Npx;"` pronto — vazio (padrão) mantém o tamanho normal do CSS. Editável no painel "Área do Site" (`templates/admin/area_do_site.html`, abaixo do upload de imagem de cada seção).
- **Barra azul do título/texto da home ("home-intro")** — desde a **parte 45** (migração `conteudo/0046`): `home_intro_titulo_tamanho_fonte`/`home_intro_texto_tamanho_fonte` (px, tamanho da letra do `<h1>`/texto, separados), `home_intro_largura` (px, largura máxima do CONTEÚDO dentro da barra — a barra em si continua sempre 100% da tela), `home_intro_altura` (px, altura mínima da barra), `home_intro_cor_fundo` (CharField, hex, editável via `<input type="color">` nativo no Django Admin — `ConfiguracaoSiteAdminForm`), `home_intro_imagem` (FileField, qualquer formato, imagem de fundo decorativa). Properties `home_intro_estilo_barra` (min-height + background-color quando NÃO há imagem), `home_intro_overlay_cor` (cor da camada semi-transparente sobre a imagem, ou `#1e3a5f` padrão), `home_intro_estilo_conteudo` (max-width do `.container` interno), `home_intro_estilo_titulo`/`home_intro_estilo_texto` (font-size). Todos opcionais — vazio mantém a aparência de sempre. Editável em Django Admin → Configuração do site → fieldset "🎨 Aparência da barra do título/texto da home".

### PaginaLivre / PaginaLivreBotao
Novos em **parte 47** (2026-07-24, migração `conteudo/0047`) — painel `/admin/paginas-livres/`. `PaginaLivre`: `titulo`, `slug` (único), `conteudo` (TextField, HTML via `RichTextWidget`, texto livre exibido no topo da página), `ativa`, `ordem`. URL própria `/pagina/<slug>/` (view `pagina_livre_detalhe` em `conteudo/views.py`) — **fora da árvore de `Categoria`** (não tem `categoria_pai`, não aparece em nenhum menu automaticamente: barra superior, Navegue por área, rodapé). É acessível só pelo link direto, colocado manualmente onde o Dan quiser (botão de categoria com URL externa, coluna extra, rodapé) — decisão confirmada com o Dan antes de implementar (ele escolheu essa opção em vez de aparecer automaticamente em algum lugar fixo do site). `PaginaLivreBotao`: `nome`, `categoria` (FK opcional, `SET_NULL` — aponta para uma categoria JÁ EXISTENTE **sem mover/reparentar nada**, mesmo padrão não-destrutivo de `ColunaExtraBotao`) OU `link_externo`, `icone`/`icone_imagem`, `icone_largura`/`icone_altura` (mesmo padrão de tamanho de ícone da parte 37) + property `icone_estilo_inline`. O botão também pode nascer como **"botão completo do site"** (checkbox no painel, mesmo mecanismo da Área do Site desde a parte 20): sem escolher categoria existente, cria uma `Categoria` RAIZ de verdade, que passa a existir em todas as árvores (Estrutura de Árvores, Painel Central, Organizador, Barra Superior) e é editável normalmente por qualquer painel depois. Template público `templates/pagina_livre_detalhe.html` reaproveita a MESMA seção de comentários (HTML/CSS/JS idênticos) de `categoria.html`/`conteudo_detalhe.html`, e o grid de botões reaproveita `.topic-grid`/`.topic-btn` (já usado no índice geral). Registrado também em Django Admin (`PaginaLivreAdmin` + inline de botões) como acesso alternativo. Nova permissão `conteudo.pode_acessar_paginas_livres`.

### Comentario
Comentários moderados (3 estados, migração 0019 — 2026-07-13). Desde a migração **0035** (parte 33, 2026-07-21), aceita comentários tanto em Conteúdo quanto em Categoria (botão) — FK dual mutuamente exclusiva, mesmo padrão do `Anexo`. Desde a migração **0047** (parte 47, 2026-07-24), aceita também Página Livre — agora é uma FK **tripla** mutuamente exclusiva.
- **`conteudo`** (FK CASCADE, **opcional** desde 0035), **`categoria`** (FK CASCADE, opcional, novo em 0035) e **`pagina_livre`** (FK CASCADE, opcional, novo em 0047) — um comentário pertence a UM dos três, nunca a mais de um; property `origem` devolve `self.conteudo or self.categoria or self.pagina_livre`. ⚠️ **Armadilha real (parte 47)**: `conteudo/teams_integration.py::_titulo_e_link()` assumia que a origem era sempre `conteudo` OU `categoria` (`if comentario.conteudo_id: ... else: comentario.categoria.nome`) — quebrava com `AttributeError` para comentários de Página Livre. Corrigido com um `elif comentario.categoria_id:` explícito. **Se adicionar um QUARTO tipo de origem no futuro, procurar por esse mesmo padrão de suposição dual em todo o código** (buscar `comentario.categoria`/`obj.categoria` fora de `admin.py`'s próprios badges já corrigidos).
...
/pagina/<slug>/                  → página livre (parte 47) — texto + botões + comentários, fora da árvore de categorias
...
10. **Lixeira** (`/admin/lixeira/`, banner vermelho escuro no dashboard, parte 41 — 2026-07-23) — botões e conteúdos excluídos por QUALQUER painel do site ficam listados aqui por 30 dias, com botões "Restaurar" (traz de volta a categoria + subárvore inteira, ou o conteúdo) e "Excluir já" (pula direto para exclusão definitiva). Passado o prazo, a limpeza automática apaga de vez sozinha (roda toda vez que a tela é aberta; também disponível via `python manage.py limpar_lixeira_expirada` / `BAT SEDU/LIMPAR LIXEIRA.bat` para quem preferir agendar). Ver seção "Lixeira" mais abaixo para detalhes técnicos.
11. **Páginas Livres** (`/admin/paginas-livres/`, banner roxo no dashboard, parte 47 — 2026-07-24) — cria páginas em branco com URL própria (`/pagina/<slug>/`), texto livre no topo, botões (categoria já existente sem mover nada, ou "botão completo do site" que cria uma categoria raiz nova) e a mesma seção de comentários de Categoria/Conteudo no rodapé. Diferente dos outros botões, NÃO aparece automaticamente em nenhum menu (barra superior, Navegue por área, rodapé) — só é acessível pelo link direto, colocado manualmente onde o Dan quiser.

## Lixeira — recuperação de exclusões em 30 dias (parte 41 — 2026-07-23)

Depois que o Dan excluiu por engano a categoria raiz "Orientações Curriculares" (18 subcategorias, 127 conteúdos) e precisou de uma recuperação manual via backup Docker/Postgres (ver histórico, item da parte 41), foi implementado um sistema de **soft-delete transparente** para `Categoria` e `Conteudo`, para que isso nunca mais exija recuperação manual.

**Como funciona por baixo dos panos**: `Categoria`/`Conteudo` ganharam campo `excluido_em` (migração `conteudo/0043`) + 2 managers cada — `.objects` (padrão, sempre filtra `excluido_em__isnull=True`, usado em TODO o resto do código sem precisar mudar nada) e `.todos_objetos` (enxerga tudo, só usado pela tela da Lixeira e pela purga automática). `Model.delete()` e `QuerySet.delete()` foram sobrescritos: em vez de apagar de verdade, só marcam `excluido_em=agora`. **Isso significa que TODOS os "excluir" que já existiam no site inteiro — Django Admin (single e bulk), Painel Central, Estrutura de Árvores, Organizador — viraram soft-delete automaticamente, sem precisar tocar no código de nenhum deles.** `Categoria.delete()` propaga recursivamente para toda a subárvore (mesmo alcance que o CASCADE real teria); `Categoria.restaurar()` desfaz tudo de uma vez. Diferente de antes: **o conteúdo dentro de uma categoria excluída NÃO fica mais órfão** (o `SET_NULL` real só dispara numa exclusão de verdade) — ele continua ligado e reaparece sozinho quando a categoria é restaurada.

**A única exclusão que continua sendo de verdade** (não vai para a lixeira): a ação "Excluir permanentemente" da Tela 2 do Painel Central (`painel/views.py`, função `conteudos_view`) chama `.hard_delete()` explicitamente — o próprio rótulo já promete que é definitivo, então foi mantido honesto.

**Onde mexer se precisar**: `conteudo/models.py` (managers `CategoriaManager`/`ConteudoManager`/`*TodosManager`, métodos `delete()`/`hard_delete()`/`restaurar()`), `conteudo/admin_views.py` (`lixeira_view`, `_purgar_lixeira_expirada()`, constante `LIXEIRA_PRAZO_DIAS = 30`), `templates/admin/lixeira.html`, `conteudo/management/commands/limpar_lixeira_expirada.py`.

⚠️ **Armadilha para lembrar**: se um dia parecer que uma categoria ou conteúdo "sumiu do banco" sem explicação, ele quase certamente está na lixeira (`Categoria.todos_objetos.filter(excluido_em__isnull=False)`), não foi apagado de verdade — confira `/admin/lixeira/` antes de qualquer recuperação manual via backup.
...
- **Usuários novos começam sem nenhum dos 9 acessos** (seguro por padrão) até alguém marcar explicitamente.

**As 9 permissões e para onde apontam:**
...
| `conteudo.pode_acessar_lixeira` | Lixeira (parte 41) |
...
14. **ATUALIZAR BANCO DOCKER.bat falhava com "DataError: value too long for type character varying(200)"** (desde 2026-07-22, corrigido com migração 0038) → campos `URLField` do Django usam 200 caracteres como default se nenhum `max_length` é especificado. SQLite ignora esse limite, mas Postgres do Docker aplica rigorosamente — qualquer link mais longo que 200 chars (comum em SharePoint/Google Drive URLs) causa erro no `loaddata`. **Solução**: migração `conteudo/0038` ajustou todos os 12 campos de URL do projeto para `max_length=1000`. Se o error aparecer novamente, rodar `python manage.py migrate conteudo 0038` local, depois `ATUALIZAR BANCO DOCKER.bat` rodará corretamente.

## Status final (2026-07-22 — v32 atualizado, pronto para futuras conversas)

✅ **Infraestrutura completa**: Site Django + Docker + PostgreSQL + Backup/Restore pronto para entregar à SEDU.

✅ **Funcionalidades**: 36 partes implementadas + 2 correções críticas após a parte 36 = **Anexo de Link/URL** (parte 36), **Botão com URL não abria direto** (correção 1), **URLFields sem max_length causavam DataError em Docker** (correção 2, migração 0038).

✅ **Documentação**: CLAUDE.md v32, 36 arquivos de memory em `C:\Users\rabalista\.claude\projects\C--ridan-Claude-Projects-Site-Curriculos-SEDU\memory\`, MEMORY.md completo com cross-links.

✅ **Migrações**: `conteudo/0012-0038` (parte 26 até correção URLFields), `painel/0002-0003`, `inteligencia/0002`. Nenhuma alteração quebrada — compatibilidade 100% para superusuários + controle granular via permissões.

✅ **Dados**: 132 categorias, 588 conteúdos, 32 categorias corrigidas retroativamente (URL redirect bug), 3674 registros totais — banco local SQLite + Postgres/Docker espelhado via `.bat`.

✅ **Testes**: Todas as 36 partes testadas ponta a ponta via Django test client. Todas as 2 correções testadas em 5 pontos administrativos. Docker sync funcionando (migração 0038 aplicada).

✅ **Painéis admin**: 9 painéis totalmente funcionais com acesso delegável por usuário/grupo.

✅ **Próximos passos** (para o Dan / futuras conversas):
- `python manage.py migrate` (aplicar migração 0038 antes de rodar ATUALIZAR BANCO DOCKER.bat)
- Deploy da SEDU em `curriculo.sedu.es.gov.br` (Dockerfile + docker-compose.yml prontos)
- SSL/TLS, restringir ALLOWED_HOSTS, DEBUG=False (ver seção Deploy)
...
15. **`ATUALIZAR BANCO DOCKER.bat` falhava com `service "web" is not running"` no passo "Aplicando migrações"** (2026-07-23, depois da parte 39) → causa: `requirements.txt` ganhou uma biblioteca nova (`python-dotenv`, para o `.env`), mas a **imagem Docker já construída** do serviço `web` não tinha sido reconstruída — o container tentava iniciar, o Django quebrava na hora de importar `settings.py` (`ModuleNotFoundError: No module named 'dotenv'`), o container morria instantaneamente, e o `.bat` via isso como "web is not running" ao tentar rodar o `migrate` dentro dele. Diagnóstico: `docker compose logs web --tail 50` mostra o traceback completo do `ModuleNotFoundError`. **Corrigido de 2 formas**: (1) reconstruídas as imagens (`docker compose build web teams_sync`) e reimportado o banco manualmente; (2) o `.bat` foi atualizado — o passo `[2/6]` agora sempre roda `docker compose build` ANTES de `docker compose up -d`, então qualquer biblioteca nova em `requirements.txt` é aplicada automaticamente na próxima vez que o Dan rodar o `.bat`, sem precisar de ajuda. **Regra geral**: sempre que uma biblioteca Python nova for adicionada ao projeto (`requirements.txt`), as imagens Docker (`web` e `teams_sync`) precisam ser reconstruídas — rodar `docker compose build` (ou deixar o `.bat` atualizado fazer isso sozinho).

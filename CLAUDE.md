# Site Currículos SEDU — Contexto do Projeto (v50 — atualizado em 2026-07-24 — Parte 50: Auto Docker + Correção Ícone de Botão)
...
- **Leva mais recente (2026-07-24 — "parte 50")**: **(1) Docker auto-inicia no Windows Startup** — novo arquivo `BAT SEDU/AUTO INICIAR DOCKER.bat` levanta os containers automaticamente sem o Dan digitar nada. Registrado no Startup folder do Windows (atalho via PowerShell) — roda toda vez que o Windows liga ou o Dan faz login. Detecta se Docker Desktop não está aberto, abre automaticamente, espera o motor ficar pronto (até 5 min), e roda `docker compose up -d`. Testado: containers subiram sozinhos na primeira rodada. **(2) Ícone de botão corrigido** — quando Dan anexa uma imagem a um botão (ex.: "Classificação Jovem Senador 2026") SEM escolher largura/altura, a imagem agora preenche/centraliza automaticamente dentro do quadrado do botão. Causa raiz: botões e "Conteúdos recentes" compartilhavam template com Conteúdo, que precisa ficar inteiro (design decision). Solução: nova classe `.icone-personalizado-botao` aplicada a subbotões + "Conteúdos recentes" em `categoria.html` e `home.html`, + novo CSS em `style.css` com `object-fit:cover` apenas para ícones de botão (Conteúdo continua `contain`). Cache-busting: `?v=20260724-3`. **Zero breaking changes** — zero migrações, zero mudanças em outras telas. Testado: botão "Classificação Jovem Senador 2026" renderiza com imagem preenchendo o quadrado corretamente.

...
- **Botão "Voltar página inicial" no rodapé** (2026-07-11): pílula discreta ENCOSTADA NO LADO DIREITO da faixa inferior do footer (a do copyright), absoluta dentro de `.footer-bottom-inner` sem deslocar o texto centralizado; no celular (≤768px) volta ao fluxo, centralizada (`.footer-voltar-inicio` em `base.html`). Pulsa com brilho BRANCO (`voltarPulse` — o pulso azul padrão sumiria no rodapé azul) APENAS fora da home, via `{% if request.path != '/' %}` (o context processor `django.template.context_processors.request` está ativo).

...
- **Botão "Voltar página inicial" no rodapé** (2026-07-11): pílula discreta no fim da coluna "Navegação" do footer (`.footer-voltar-inicio` em `base.html`). Pulsa com brilho BRANCO (`voltarPulse` — o pulso azul padrão sumiria no rodapé azul) APENAS fora da home, via `{% if request.path != '/' %}` (o context processor `django.template.context_processors.request` está ativo).

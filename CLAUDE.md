...
- Versão de cache do CSS evoluiu ao longo do dia: `?v=20260711-1` → `-2` (pílula do Currículo Atual + recentes quadrados) → `-3` (ícone personalizado em Conteudo, cards compactos, tamanho de botão) → `-4` (ícone personalizado em Categoria/botões) → `-5` (correção do ajuste de imagem — cover em vez de contain nos botões). JS ficou em `?v=20260711-1`.
- Testado: páginas 200, ações do painel via test client, breakpoints 1280/1160/960/375px, menu ⋯ funcionando, submissão completa do Painel Central com tipo de conteúdo/vídeo/ícone-imagem/tamanho, upload de ícone de botão em lote para categoria pai + subcategoria simultaneamente (confirmado que ambos os arquivos gravam com o tamanho completo, sem truncamento), ajuste de imagem verificado via computed style nos 3 locais (category-icon 30x30, topic-icon 42x42, chip 22x22) com `object-fit:cover` e `overflow:hidden` aplicados corretamente.
